...
public class CommonLib {
	...
	public static void InitializeDriverObj() throws Exception {
		...
		case "Chrome":
			...
			// "Resources/Driver/chrome_driver/chromedriver.exe");
			// cap.setBrowserName("chrome");
			// cap.setPlatform(Platform.ANY);

			// ChromeOptions options = new ChromeOptions();
			// options.addArguments("user-data-dir=/path/to/profile/if/needed");
			// opsChrome.setBinary("Resources/Driver/chrome_driver/chromedriver.exe");
			// cap.setCapability(ChromeOptions.CAPABILITY, opsChrome);
			// sDriver = new RemoteWebDriver(new
			// URL("http://192.168.104.92:4444/wd/hub"), cap);
		...
		sDriver.manage().timeouts().pageLoadTimeout(20, TimeUnit.SECONDS);
	...
	public static void clickButton(By elementLocator) {
		String txt = "";
		try {
			txt = sDriver.findElement(elementLocator).getText();
			if (txt.equals("") || txt == null) {
				txt = sDriver.findElement(elementLocator).getAttribute("alt");
			}
			if (txt.equals("") || txt == null) {
				txt = sDriver.findElement(elementLocator).getAttribute("id");
			}
		} catch (Exception e) {
			txt = elementLocator.toString();
		}

		// String id = sDriver.findElement(elementLocator).getAttribute("id");
		...
		staticWait(1);
		CustomReporter.MessageLogger(logFormator(txt) + " : " + " Clicked !", CustomReporter.status.Information);
		// CustomReporter.MessageLogger(" with Text " + txt + " : " + " Clicked
		// !", CustomReporter.status.Information);
	}
	...
	public static String selectDojoListByXpath(String xpath, String id) throws InterruptedException {
		...
		Random rand = new Random();
		int randomNumber = rand.nextInt(finalcount) + 2;
		for (int intC = 0; intC < randomNumber; intC++) {
			staticWait(2);
			listObject.sendKeys(Keys.ARROW_DOWN);
		}
