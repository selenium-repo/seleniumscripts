...
import pharmacy.pages.PatientSearchPage;
...
public class PatientSearchTestCases extends PatientSearchPage {
	...
	public void verifyPatientSearchByNumber() {
		...
		if (Config.props.getProperty("PatNum").equals(HashTableRepository.getHash("pat_num"))) {
			CustomReporter.MessageLogger("Rx Patient Search by Patient Number " + HashTableRepository.getHash("pat_num")
					+ " is working successfully, which is as expected", CustomReporter.status.Pass);
		} else {
			CustomReporter.MessageLogger(
					"Failed to do the Rx Patient Search by Patient Number, please try again, which is not as expected",
	...
	public void verifyPatientSearchByName() {
		...
		if (first_name1.equals(first_name2) && last_name1.equals(last_name2)) {
			CustomReporter.MessageLogger("Rx Patient Search by Patient Name "
					+ HashTableRepository.getHash("patient_name") + " is working successfully, which is as expected",
					CustomReporter.status.Pass);
		} else {
			CustomReporter.MessageLogger(
					"Failed to do the Rx Patient Search by Patient Name, please try again, which is not as expected",
					CustomReporter.status.Fail);
	...
	public void verifyPatientSearchByMedRecNo() {
		...
		if (pat_med_rec_no.equals(med_rec_no)) {
			CustomReporter.MessageLogger("Rx Patient Search by Patient Medical Record Number "
					+ HashTableRepository.getHash("med_record_no") + " is working successfully, which is as expected",
					CustomReporter.status.Pass);
		} else {
			CustomReporter.MessageLogger(
					"Failed to do the Rx Patient Search by Patient Medical Record Number, please try again, which is not as expected",
					CustomReporter.status.Fail);
		}
	...
	public void verifyPatientSearchByDOB() {
		...
		if (HashTableRepository.getHash("pat_dob").equals(dob_txt)) {
			CustomReporter.MessageLogger("Rx Patient Search by Patient Date Of Birth "
					+ HashTableRepository.getHash("pat_dob") + " is working successfully, which is as expected",
					CustomReporter.status.Pass);
		} else {
			CustomReporter.MessageLogger(
					"Failed to do the Rx Patient Search by Patient DOB, please try again, which is not as expected",
					CustomReporter.status.Fail);
		}
	...
	public void verifyNursingStationSearch() {
		...
		if (count >= 1) {
			if (checkForErros()) {
				CustomReporter.MessageLogger(
						"Rx Search By Nursing Station is working successfully, which is as expected",
						CustomReporter.status.Pass);
			} else {
				CustomReporter.MessageLogger(
						"Failed to do the Rx Search By Nursing Station, please try again, which is not as expected",
						CustomReporter.status.Fail);
			}

		} else {
			CustomReporter.MessageLogger(
					"Rx Search By Nursing Station is working properly, but there is no data for the selected nursing station",
